...
# 读取数据
file_path = r"D:\Study on Youth Suicide Rate\cla_injuryresult.csv"
...
# 定义类别标签
class_labels = ["身体欺凌型", "言语欺凌型", "低欺凌暴露型", "关系欺凌型"]
...
# 保存饼图
plt.savefig("D:\Study on Youth Suicide Rate\injury_class_proportion_pie.png", dpi=500)
plt.show()

# 2. 绘制折线图：四类同学在前三列指标上的表现
# 计算每类在前三列指标上的均值
class_means = data.groupby('Class')[['言语欺凌', 
                                    '财物欺凌', 
                                    '身体欺凌']].mean()
...
# 保存折线图
plt.savefig("D:\Study on Youth Suicide Rate\图5四类同学在霸凌倾向指标上的表现.png", dpi=600)
plt.show()
